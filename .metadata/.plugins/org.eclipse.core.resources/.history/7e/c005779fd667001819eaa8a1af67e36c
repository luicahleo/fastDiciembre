...
public class ServletModificarUsuario extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (error) {//"Si hay algún error debe reenviar la petición a la página de error (error.jsp), estableciendo previamente
			...
			request.setAttribute("volverURL", "/admin/usuForm.jsp");//Desde qué pagina se accede a éste Servlet?==Este servlet es accedido a través de la página verCrearInterfaces.jsp, está en usuarios(Folder)
