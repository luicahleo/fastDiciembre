...
function ipToBin(cadena) {
	var ip = cadena.split('.');// ip=array de los elementos que hay separados
								// por puntos
	var res = "";

	for (var i = 0; i < ip.length; i++) {
		ip[i] = (parseInt(ip[i])).toString(2).padStart(8, 0);// paso a
																// binario
...
/*
 * var a = num.toString(2); var b = num.toString(2); var c = num.toString(2);
...
function mostrarRedes(redes) {
	...
	document.getElementsByTagName("table")[0].innerHTML = tabla;// pritnf de la
																// tabla
...
function pedirRedesURL() {
	...
	if (tabla.hasChildNodes() == false) { // Si no hay filas en la tabla
											// (aparte del
		// título) entonces cambia el valor del
		// boton y muestra la tabla
		// document.getElementById('mostrar').innerHTML = "<input type='button'
		// onclick='pedirRedesURL()' value='Ocultar Redes'>";
		// esta bn pq cambia justo al darle a mostrar redes por primera vez. sin
		// eliminar filas
		...
		document.getElementById('mostrar').innerHTML = "Mostrar Redes";
		// esta bn pq sale cuando le das al boton y se va cambiando a
		// mostrar/ocultar. sin eliminar filas
...
function inicial() {
	...
	// pedirRedesURL()); eror pq habia que poner function
}
